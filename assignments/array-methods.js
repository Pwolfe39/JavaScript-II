...
// });
let fullName = [];
runners.forEach((name) => {fullName.push(`First: ${name.first_name} Last: ${name.last_name}`)});
...
// Problem 1
// Get the company names and their donations
let companyNameAndDonation = [];
runners.forEach((companyDonations) => {companyNameAndDonation.push(`Co: ${companyDonations.company_name}, Donations: ${companyDonations.donation}`)});
console.log(companyNameAndDonation);

// Problem 2
//
let xlargeShirts = runners.filter(tShirts => tShirts.shirt_size === 'L' || tShirts.shirt_size === 'XL');
console.log(xlargeShirts);

// Problem 3
let test = runners.map((person) => {
    console.log(`First Name:${person.first_name} Last Name:${person.last_name} Email:${person.email}`);
})